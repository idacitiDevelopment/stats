...
$result = $db->select("select date(view_card.creation_time) as created,count(view_card.id) as cards
from view_card
where DATE(view_card.creation_time) > DATE_SUB(CURDATE(), INTERVAL 90 DAY)
group by date(view_card.creation_time)
");
...
$result = $db->select("select date_format(view_card.creation_time,'%v-Week-%M') as created,count(id) as cards
from view_card
where DATE(view_card.creation_time) > DATE_SUB(CURDATE(), INTERVAL 90 DAY)
group by weekofyear(view_card.creation_time)
");
while ($row = $db->get_row($result, 'MYSQL_ASSOC')) {
	$created = stripslashes ($row["created"]);
	$cards = stripslashes ($row["cards"]);
...
$result = $db->select("select date_format(view_card.creation_time,'%m-%M') as created,count(id) as cards
from view_card
where DATE(view_card.creation_time) > DATE_SUB(CURDATE(), INTERVAL 365 DAY)
group by month(view_card.creation_time)
");
while ($row = $db->get_row($result, 'MYSQL_ASSOC')) {
	$created = stripslashes ($row["created"]);
	$cards = stripslashes ($row["cards"]);
  ...
  <div class="content">
    ...
    <div class="content-container">
      ...
      <div class="content-header">
        <h2 class="content-header-title">Card Views</h2>
        <ol class="breadcrumb">
