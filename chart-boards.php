...
$result = $db->select("select date(creation_time) as created,count(id) as boards
from storyboard
...
while ($row = $db->get_row($result, 'MYSQL_ASSOC')) {
	...
	$byday_array[$count] = array ($created,$boards);
				$count ++;}

//CHART BY WEEK IN QTR
$result = $db->select("select date_format(creation_time,'%v-Week-%M') as created,count(id) as boards
from storyboard
where DATE(creation_time) > DATE_SUB(CURDATE(), INTERVAL 90 DAY)
...
while ($row = $db->get_row($result, 'MYSQL_ASSOC')) {
	$created = stripslashes ($row["created"]);
	$boards = stripslashes ($row["boards"]);
	$byqtr_array[$count2] = array ($created,$boards);
				$count2 ++;}

//CHART BY MONTH IN YEAR
$result = $db->select("select date_format(creation_time,'%m-%M') as created,count(id) as boards
from storyboard
...
while ($row = $db->get_row($result, 'MYSQL_ASSOC')) {
	$created = stripslashes ($row["created"]);
	$boards = stripslashes ($row["boards"]);
	$byyear_array[$count3] = array ($created,$boards);
				$count3 ++;}
?>
  ...
  <div class="content">
    ...
    <div class="content-container">
      ...
      <div class="content-header">
        <h2 class="content-header-title">Storyboards Created</h2>
        <ol class="breadcrumb">
          <!--<li><a href="./index.html">Home</a></li>
          <li><a href="javascript:;">Extra Pages</a></li>
          <li class="active">User Storyboards Created</li>-->
        </ol>
      </div> <!-- /.content-header -->
...
<div class="portlet">
            ...
            <div class="portlet-header">
              ...
              <h3>
                ...
                User Storyboard Creation Timelines
              </h3>
               </div> <!-- /.portlet-header -->
...
Morris.Line({
  ...
  data: [
      <?php foreach($byday_array as $value){ ?>
        { created:'<?php echo $value[0]; ?>',boards: <?php echo $value[1]; ?> },
   <?php } ?>
         ],
  xkey: 'created',
  ykeys: ['boards'],
  labels: ['Storyboards','Date'],
  lineColors: ['#4da74d']
